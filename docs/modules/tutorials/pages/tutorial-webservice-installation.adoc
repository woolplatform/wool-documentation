...
The use of an external variable service can be enabled by setting `woolconfigExternalVariableServiceEnabled` to `true`.
...
* `woolconfigExternalVariableServiceAPIVersion` should be set to the API version for the external variable service (e.g., `1.0.0`).
* `woolconfigExternalVariableServiceApiToken` can be set to an API token if the external service requires authentication (currently not supported yet).
