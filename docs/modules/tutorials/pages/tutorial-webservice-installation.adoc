...
The use of an external variable service can be turned on by setting `woolconfigExternalVariableServiceEnabled` to 'true'. 
...
* `woolconfigExternalVariableServiceApiToken` can be set to an API token if the external service requires authentication. 
